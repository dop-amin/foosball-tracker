...
class Game(db.Model):
    ...
    @property
    def is_shutout(self):
        return self.team1_score == 10 and self.team2_score == 0 or self.team1_score == 0 and self.team2_score == 10
...
def get_game_form():
    ...
    players = Player.query.order_by(Player.name).all()
    return render_template('partials/game_form.html', game_type=game_type, players=players)
...
def add_game():
    ...
    try:
        ...
        start_time = parse(request.form.get('start_time')) if request.form.get('start_time') else None
        end_time = parse(request.form.get('end_time')) if request.form.get('end_time') else None
...
def get_leaderboard():
    ...
    players_stats.sort(key=lambda x: (x['win_rate'], x['goal_difference']), reverse=True)
...
def get_cake_leaderboard():
    ...
    cake_stats = db.session.query(
        Player.name,
        db.func.sum(CakeBalance.balance).label('total_cakes')
    ).join(
        CakeBalance, Player.id == CakeBalance.creditor_id
    ).group_by(
        Player.id, Player.name
    ).order_by(
        db.func.sum(CakeBalance.balance).desc()
    ).all()
    
    # Get players who owe the most cakes
    debt_stats = db.session.query(
        Player.name,
        db.func.sum(CakeBalance.balance).label('total_debt')
    ).join(
        CakeBalance, Player.id == CakeBalance.debtor_id
    ).group_by(
        Player.id, Player.name
    ).order_by(
        db.func.sum(CakeBalance.balance).desc()
    ...
    return render_template('partials/cake_leaderboard.html', 
...
def get_win_rates():
    ...
    for player in players:
        ...
        for game_type in game_types:
            ...
            games_played = db.session.query(GamePlayer).join(Game).filter(
                GamePlayer.player_id == player.id,
                Game.game_type == game_type
            ).count()
            
            games_won = db.session.query(GamePlayer).join(Game).filter(
                GamePlayer.player_id == player.id,
                Game.game_type == game_type,
    ...
    return render_template('partials/win_rates.html', win_rates=win_rates, game_types=game_types)
...
def get_detailed_stats():
    ...
    games_with_duration = Game.query.filter(Game.start_time.isnot(None), Game.end_time.isnot(None)).all()
    avg_duration = None
    if games_with_duration:
        avg_duration = sum(g.duration_minutes for g in games_with_duration) / len(games_with_duration)
    
    # Most active players
    most_active = db.session.query(
        Player.name,
        db.func.count(GamePlayer.id).label('game_count')
    ).join(GamePlayer).group_by(Player.id, Player.name).order_by(
        db.func.count(GamePlayer.id).desc()
    ).limit(3).all()
    
    # Best winning streaks (simplified - just current win rate leaders)
    best_players = db.session.query(
        Player.name,
        db.func.count(GamePlayer.id).label('total_games'),
        db.func.sum(db.case((GamePlayer.is_winner == True, 1), else_=0)).label('wins')
    ).join(GamePlayer).group_by(Player.id, Player.name).having(
        db.func.count(GamePlayer.id) >= 3
    ).all()
    ...
    ).group_by(Game.game_type).order_by(
